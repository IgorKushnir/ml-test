...
<!--            <div v-if="discontinued" class="badge light_bg p-h-16 p-v-8 gray">Discontinued</div>-->
              ...
              <div>
                ...
                <div class="p-small">
                  <template v-for="(color, index) in data.colors.data">
                    <NuxtLink :to="data?.type?.data?.attributes?.slug === 'dress' || data?.type?.data?.attributes?.slug === 'suknie' ? localePath(`/${(data?.type?.data?.attributes?.slug)}/${routeLocalization[locale].colors}/${color?.attributes?.slug}`) : ''" class="p-small link">{{ color?.attributes?.title }}</NuxtLink><span v-if="index + 1 < data.colors.data?.length">, </span>
                  </template>
                </div>
              </div>
            </div>
            ...
            <div v-if="data?.silhouettes?.data" class="info m-b-32  m-b-16-md">
              <div>
                ...
                <div class="p-small">
                  <NuxtLink :to="data?.type?.data?.attributes?.slug === 'dress' || data?.type?.data?.attributes?.slug === 'suknie' ? localePath(`/${(data?.type?.data?.attributes?.slug)}/${routeLocalization[locale].silhouettes}/${data.silhouettes.data.attributes.slug}`) : ''" class="p-small link">{{ data.silhouettes.data?.attributes?.title }}</NuxtLink>
                </div>
              </div>
            </div>
              ...
              <div>
                ...
                <div class="p-small">
                  <template v-for="(fabric, index) in data.fabrics.data">
                    <NuxtLink :to="data?.type?.data?.attributes?.slug === 'dress' || data?.type?.data?.attributes?.slug === 'suknie' ? localePath(`/${(data?.type?.data?.attributes?.slug)}/${routeLocalization[locale].fabrics}/${fabric.attributes.slug}`) : ''" class="p-small link">{{ fabric.attributes.title }}</NuxtLink><span v-if="index+1 < data.fabrics.data?.length">, </span>
                  </template> 
                </div>
              </div>
            </div>
